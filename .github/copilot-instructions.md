...
- **Natural-recovery stability verification**: after the GMSL link appears alive post-HW-reset (`DS5_DEVICE_TYPE` readable), always wait for the FW secondary init window (`DS5_HW_RESET_NATURAL_SETTLE_MS`) and re-verify the link with `ds5_read_poll()` before declaring success. FW secondary init reconfigures the serializer ~200-350ms after initial readiness and can kill the link. Escalate to full SERDES recovery if any post-settle check fails.
