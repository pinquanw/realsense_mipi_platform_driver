...
- **Polling vs. normal I2C semantics**: for transient-failure-expected polling loops (HWMC status checks, reset readiness polls, SERDES recovery probes, DFU timeout checks), use `ds5_read_poll()` / `ds5_write_poll()` instead of `ds5_read()` / `ds5_write()`. The poll variants perform a single direct `regmap_raw_read/write()` call without retry or logging, preventing false warnings and excessive log spam on expected transients. Reserve `ds5_read()` / `ds5_write()` for normal I2C operations where retry logic and verbose failure logging are desired.
...
- **Post-reset GMSL write-path validation**: `ds5_wait_device_type()` validates both read AND write paths — after `DS5_DEVICE_TYPE` becomes readable it probes `control_status_reg` with a scratch write using the remaining retry budget. After HW reset the GMSL tunnel can return stale/garbage data on reads (no error) while writes fail with `-EREMOTEIO` (`-121`); read-only checks cannot detect this. All callers get write-path validation automatically.
