...
- **Post-reset GMSL write-path validation**: after any GMSL link recovery (including "natural" recovery when `DS5_DEVICE_TYPE` becomes valid without SERDES intervention), verify the I2C **write** path before proceeding. After HW reset the GMSL tunnel can return stale/garbage data on reads (no error) while writes fail with `-EREMOTEIO` (`-121`). Read-only checks cannot detect this. Probe with a scratch-register write; poll up to `DS5_HW_RESET_STABILITY_TIMEOUT_MS`; if writes still fail, escalate to `ds5_hw_reset_serdes_recovery()`.
