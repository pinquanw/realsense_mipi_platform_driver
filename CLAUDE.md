...
- After natural recovery (DEVICE_TYPE readable immediately post-reset), wait for the FW secondary init window (`DS5_HW_RESET_NATURAL_SETTLE_MS`) before declaring success. FW secondary init reconfigures the serializer ~200-350ms after initial readiness and can kill the link. Re-verify with `ds5_read_poll()` after the settle window; escalate to full SERDES recovery if any check fails.
