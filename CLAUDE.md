...
- For polling loops expecting transient I2C failures (HWMC status checks, reset readiness polls, SERDES recovery probes, DFU timeout checks), use `ds5_read_poll()` / `ds5_write_poll()` which perform a single-shot regmap read/write without retry or logging. This prevents false warnings and excessive log spam. Reserve `ds5_read()` / `ds5_write()` for normal I2C operations where retry semantics are desired.
